...
char *strupr(char *s);//to change anystring given to uppercase
void passwdhide(char buffer[]);// To input password from user so that asterik (*) is displayed for each character entered. Similar to gets(string); function
void increase(char i_id[]);
void currency(char cur[], float n); 
void numtowrd(double n,char word[]);// To convert a floating (double) number n into word according to Hindu Arabic Number System
...
#define DASH for(i = 0; i < 78; i++) printf("_")
...
//Log in Screen begins...
    ...
    do
    {
        ...
        {
            title();
            printf("Invalid User name or Password!\a");
...
void admin()
{
    ...
    do{
       ...
       // char pass[30]={0};
        ...
        if(strcmp(uname,"ADMIN")==0 && strcmp(pass,"IOE") == 0)
            c=1;
        else
        {
            title();
